...
public class TreeTest {
    ...
    public void testGraph001() throws IOException {
        ...
        Edges solution = Tree.solve(graph);
    ...
    @Test
    public void testGraph023() throws IOException {
        Graph graph = GraphReader.readGraph("graph023.txt", edgesReader);
        Edges solution = Tree.solve(graph);
        Assert.assertEquals(graph.getSolution().size(), solution.size());
        Assert.assertTrue(graph.checkSolution(solution));
        System.out.println(String.format("Graph-023: %s", solution));
    }
}
